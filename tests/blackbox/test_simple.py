...
def test_connect(docker_client):
    with SnowGlobeService.run(docker_client) as service, connector.connect(**service.local_connection_kwargs()) as conn:
...
async def test_simultaneous_connections(docker_client):
    async with SnowGlobeService.arun(docker_client) as service:
        ...
        with connector.connect(**service.local_connection_kwargs()) as conn:
            ...
            results = conn.cursor().execute("select x, y from bar where y like 't%'").fetchall()
            assert results == [(2, "two"), (3, "three")]
            with connector.connect(**service.local_connection_kwargs()) as conn2:
                conn2.cursor().execute("use database foo;")
                conn2.cursor().execute("use schema loolie")
                results = conn2.cursor().execute("select x, y from bar where y like 't%'").fetchall()
                assert results == [(2, "two"), (3, "three")]
